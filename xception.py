...
# cSpell:ignore monkeypox imshow
...
monkeypox_images = random.sample(os.listdir(os.path.join(test_dir, 'monkeypox')), 5)
others_images = random.sample(os.listdir(os.path.join(test_dir, 'others')), 5)

# Load and plot the selected images with labels
selected_images = monkeypox_images + others_images
selected_labels = ['monkeypox'] * 5 + ['others'] * 5

# Load images
images = []
for img_path in selected_images:
    img = plt.imread(os.path.join(test_dir, 'monkeypox' if img_path in monkeypox_images else 'others', img_path))
    images.append(img)
